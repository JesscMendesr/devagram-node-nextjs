...
export const conectarMongoDB =
  ...
  async (req: NextApiRequest, res: NextApiResponse<RespostaPadraoMsg>) => {
    // verificar se o banco está conectado, se estiver, seguir para o endpoint ou ´
    // próximo middleware.
    ...
    if (!DB_CONEXAO_STRING) {
      return res
        .status(500)
        .json({ erro: "ENV de configuração do banco, nao informado" });
    }
    mongoose.connection.on("connected", () =>
      console.log("Banco de dados conectado")
    );
    mongoose.connection.on("error", (error) =>
      console.log(`Ocorreu erro ao connectar no banco: ${error}`)
    );
